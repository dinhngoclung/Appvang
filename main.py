...
# ===================================================================
# GIAO DIỆN BẢN AN TOÀN (CHỈ LÀM VIỆC NỘI BỘ APP COZY)
# ===================================================================
def main(page: ft.Page):
    ...
    # Thư mục sandbox riêng tư của app, 100% không bị chặn quyền
    target_dir = os.getcwd()

    title_text = ft.Text("TOOL TÁCH SHEET EXCEL (BẢN AN TOÀN)", style=ft.TextThemeStyle.HEADLINE_SMALL, weight=ft.FontWeight.BOLD)
    
    info_text = ft.Text(f"Vị trí thư mục App:\n{target_dir}", color="blue", size=13)
    ...
    def quet_tim_file_excel(e=None):
        log_box.value = ""
        write_log("🔍 Đang tìm file .xlsx trong bộ nhớ nội bộ...")
        try:
            ...
            if cac_file:
                ...
                write_log(f"-> Tìm thấy {len(cac_file)} file. Đã chọn file mới nhất: {cac_file[0]}")
            else:
                file_dropdown.options = []
                file_dropdown.value = None
                file_dropdown.disabled = True
                btn_run.disabled = True
                write_log("⚠️ Chưa có file Excel nào trong thư mục này!\nHãy copy file Excel của bạn vào đây.")
    ...
    def bat_dau_xu_ly(e):
        ...
        try:
            ...
            write_log("====================================")
            write_log(f"File kết quả xuất ra thành công:\n-> {output_name}")
            quet_tim_file_excel()
    ...
    # Bản này tự động gọi thoải mái vì nằm trong thư mục app, bao lên hình mượt mà!
    quet_tim_file_excel()
