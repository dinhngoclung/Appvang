...
# ===================================================================
# GIAO DIỆN ĐỒ HỌA ỨNG DỤNG (BẢO HIỂM CHỐNG SẬP MÀN HÌNH XÁM)
# ===================================================================
def main(page: ft.Page):
    ...
    log_box = ft.TextField(
        ...
        max_lines=15, 
        read_only=True,
        value="HƯỚNG DẪN:\n1. Chọn thư mục phía trên.\n2. Bấm nút 'BẤM ĐỂ QUÉT FILE' để tìm file Excel.\n"
    )
    ...
    def quet_tim_file_excel(e=None):
        ...
        # Bọc try-except toàn bộ để nếu hệ thống chặn quyền cũng không bị sập app
        try:
            if not os.path.exists(target_dir):
                write_log("❌ Thư mục không tồn tại hoặc App chưa được cấp quyền bộ nhớ!")
            ...
            write_log(f"❌ KHÔNG QUÉT ĐƯỢC THƯ MỤC!\nLý do: {ex}\n\n👉 Bạn cần bật quyền bộ nhớ cho App trong cài đặt điện thoại.")
            file_dropdown.options = []
            file_dropdown.disabled = True
            btn_run.disabled = True
        page.update()
    ...
    # KHÔNG gọi quet_tim_file_excel() tự động ở đây nữa để tránh sập app lúc mới mở
