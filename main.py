...
# ===================================================================
# GIAO DIỆN ĐỒ HỌA ỨNG DỤNG (BẢN CHỌN THƯ MỤC & ĐƯỜNG DẪN TÙY CHỌN)
# ===================================================================
def main(page: ft.Page):
    page.title = "Tool Tach Sheet Thong Minh"
    page.scroll = ft.ScrollMode.AUTO
    page.padding = 15

    # Các thư mục mặc định trên Android
    base_storage = "/storage/emulated/0"
    folder_presets = {
        "Thư mục Download (Tải về)": f"{base_storage}/Download",
        "Thư mục Zalo nhận file": f"{base_storage}/Android/media/com.zing.zalo/Zalo/Attachment",
        "Thư mục Documents": f"{base_storage}/Documents",
        "Thư mục Telegram": f"{base_storage}/Telegram/Telegram Documents",
        "Thư mục Bluetooth": f"{base_storage}/Bluetooth",
        "Thư mục hiện tại của App": os.getcwd()
    }

    title_text = ft.Text("TOOL TÁCH SHEET EXCEL ĐA THƯ MỤC", style=ft.TextThemeStyle.HEADLINE_SMALL, weight=ft.FontWeight.BOLD)
    
    # Dropdown để chọn nhanh thư mục mẫu
    folder_dropdown = ft.Dropdown(
        label="Chọn nhanh thư mục quét",
        options=[ft.dropdown.Option(key, text=key) for key in folder_presets.keys()],
        value="Thư mục Download (Tải về)"
    )

    # Ô nhập text cho phép b tự sửa/dán đường dẫn nếu muốn
    path_input = ft.TextField(
        label="Đường dẫn thư mục đang chọn (Có thể tự sửa)",
        value=folder_presets["Thư mục Download (Tải về)"],
        multiline=False
    )

    # Thả xuống chọn file Excel tìm được trong thư mục đó
    file_dropdown = ft.Dropdown(
        label="Chọn file Excel cần xử lý",
        options=[],
        disabled=True
    )
    ...
    # Thay đổi lựa chọn trong Dropdown thư mục mẫu
    def on_preset_change(e):
        selected_key = folder_dropdown.value
        if selected_key in folder_presets:
            path_input.value = folder_presets[selected_key]
            write_log(f"Chuyển sang: {selected_key}")
            quet_tim_file_excel()
        page.update()

    folder_dropdown.on_change = on_preset_change

    # Hàm quét file Excel trong thư mục đã chỉ định
    def quet_tim_file_excel(e=None):
        target_dir = path_input.value.strip()
        log_box.value = "" # Reset log khi quét mới
        write_log(f"🔍 Đang quét tại: {target_dir}")
        
        if not os.path.exists(target_dir):
            write_log("❌ Đường dẫn không tồn tại hoặc App chưa được cấp quyền truy cập bộ nhớ này!")
        ...
        try:
            ...
            if cac_file:
                ...
                file_dropdown.options = [ft.dropdown.Option(f, text=f) for f in cac_file]
                file_dropdown.value = cac_file[0] # Tự chọn file mới nhất
                file_dropdown.disabled = False
                btn_run.disabled = False
                write_log(f"-> Tìm thấy {len(cac_file)} file Excel. Đã chọn file mới nhất: {cac_file[0]}")
            else:
                file_dropdown.options = []
                file_dropdown.value = None
                file_dropdown.disabled = True
                btn_run.disabled = True
                write_log("⚠️ Không tìm thấy file .xlsx nào trong thư mục này!")
        except Exception as ex:
            write_log(f"❌ Lỗi quét thư mục: {ex}")
            file_dropdown.options = []
            file_dropdown.disabled = True
            btn_run.disabled = True
        page.update()
    ...
    def bat_dau_xu_ly(e):
        ...
        if not target_dir or not selected_file:
            write_log("!!! Không có file hoặc thư mục hợp lệ để xử lý.")
            return
        ...
        try:
            write_log(f"\n[1] Bắt đầu đọc file: {selected_file}")
            
            df_all = pd.read_excel(file_path, header=None, dtype=str)
            write_log(f" -> OK: Tìm thấy {df_all.shape[0]} dòng, {df_all.shape[1]} cột")
            ...
            if header_row is None:
                header_row = 0
                write_log(" -> Không tìm thấy dòng tiêu đề, tự động lấy dòng số 1")
            else:
                write_log(f" -> Đã định vị dòng tiêu đề (Dòng {header_row + 1})")
            ...
            write_log(f"[2] Lọc dữ liệu sạch: Còn {len(df)} dòng hợp lệ")
            ...
            # Xuất file kết quả vào ngay thư mục đang chọn
            output_name = f"OKKK-T{thang}-{nam}.xlsx"
            output_path = os.path.join(target_dir, output_name)
            
            write_log(f"[3] Đang xuất file Excel kết quả...")
            ...
            write_log("\n====================================")
            write_log("🎉 TÁCH SHEET THÀNH CÔNG RỒI B ƠI! 🎉")
            write_log("====================================")
            write_log(f"File kết quả nằm cùng thư mục bạn chọn:\n-> {output_name}")
            
            # Quét lại để cập nhật danh sách (loại file kết quả vừa tạo nếu có quét lại)
            quet_tim_file_excel()
    ...
    # Quét thư mục mặc định ngay lần đầu chạy app
    quet_tim_file_excel()
