...
# ===================================================================
# GIAO DIỆN ĐỒ HỌA ỨNG DỤNG (ĐỊNH VỊ THƯ MỤC DOWNLOAD CỦA ANDROID)
# ===================================================================
def main(page: ft.Page):
    page.title = "Tool Tach Sheet Download"
    page.scroll = ft.ScrollMode.AUTO
    page.padding = 20

    title_text = ft.Text("TOOL TACH SHEET EXCEL (THU MUC DOWNLOAD)", weight=ft.FontWeight.BOLD)
    selected_file_text = ft.Text("Dang quet tim file Excel...", italic=True)
    ...
    # Hàm tìm đường dẫn thư mục Download chuẩn trên Android độc lập
    def get_download_folder():
        # Thử đường dẫn lưu trữ chung phổ biến nhất của Android
        paths = [
        ...
        for p in paths:
            if os.path.exists(p):
                return p
        return os.getcwd() # Phương án dự phòng

    def bat_dau_xu_ly(file_path):
        if not file_path:
            write_log("!!! Khong co file de xu ly.")
            return
        ...
        try:
            write_log(f"[1] Doc file: {os.path.basename(file_path)}")
            
            df_all = pd.read_excel(file_path, header=None, dtype=str)
            write_log(f" -> OK: File co {df_all.shape[0]} dong, {df_all.shape[1]} cot")
            ...
            if header_row is None:
                header_row = 0
                write_log(" -> Khong tim thay header ro rang, su dung dong dau tien")
            else:
                write_log(f" -> Tim thay header o dong {header_row + 1}")
            ...
            write_log(f"[2] Loc xong: {len(df)} dong hop le")
            ...
            # Xuất thẳng file vào thư mục Download
            output_name = f"OKKK-T{thang}-{nam}.xlsx"
            output_dir = get_download_folder()
            output_path = os.path.join(output_dir, output_name)
            
            write_log(f"\n[3] Dang ghi file ket qua ra thu muc Download...")
            ...
            write_log("\n====================================")
            write_log("🎉 THANH CONG ROI B OI! 🎉")
            write_log("====================================")
            write_log(f"File da luu tai thu muc Download cua may:\n-> {output_name}")
    ...
    def quet_tim_file_excel(e=None):
        try:
            thu_muc_download = get_download_folder()
            write_log(f"Thu muc quet: {thu_muc_download}")
            
            # Tìm các file xlsx trong thư mục Download (loại trừ file kết quả OKKK)
            cac_file = [f for f in os.listdir(thu_muc_download) if f.endswith('.xlsx') and not f.startswith('OKKK')]
            
            if cac_file:
                # Ưu tiên lấy file mới được chỉnh sửa gần nhất
                cac_file.sort(key=lambda x: os.path.getmtime(os.path.join(thu_muc_download, x)), reverse=True)
                file_target = cac_file[0]
                
                current_file_path[0] = os.path.join(thu_muc_download, file_target)
                selected_file_text.value = f"Tim thay file moi nhat: {file_target}"
                btn_run.disabled = False
                write_log(f"-> Da nap file: {file_target}")
            else:
                selected_file_text.value = "CHƯA THẤY FILE! Hãy chep file Excel vao thu muc Download (Tai ve) cua dien thoai."
                btn_run.disabled = True
        except Exception as ex:
            selected_file_text.value = f"Loi quet file: {ex}"
        page.update()

    btn_refresh = ft.ElevatedButton(
        "Quet lai thu muc Download (Refresh)",
