...
# ===================================================================
# GIAO DIỆN SỬA LỖI TRIỆT ĐỂ CHỐNG TRẮNG MÀN HÌNH
# ===================================================================
def main(page: ft.Page):
    ...
    # Ép giao diện cập nhật ngay lập tức để tránh lỗi đứng hình
    page.update()

    target_dir = os.getcwd()

    title_text = ft.Text("TOOL TÁCH SHEET EXCEL (BẢN FIX TRẮNG MÀN)", style=ft.TextThemeStyle.HEADLINE_SMALL, weight=ft.FontWeight.BOLD)
    info_text = ft.Text(f"Thư mục App: {target_dir}", color="blue", size=13)

    file_dropdown = ft.Dropdown(
        label="Chọn file Excel trong ứng dụng",
        options=[],
        disabled=True
    )
    ...
    def quet_tim_file_excel(e=None):
        log_box.value = ""
        write_log("🔍 Đang tìm file .xlsx trong bộ nhớ...")
        try:
            ...
            if cac_file:
                ...
                write_log("⚠️ Chưa có file Excel (.xlsx) nào!\nHãy copy file Excel của bạn vào thư mục này.")
        except Exception as ex:
            write_log(f"❌ Lỗi quét thư mục: {ex}")
        page.update()

    def delayed_scan():
        # Trì hoãn 1.2 giây chờ giao diện vẽ xong hoàn toàn mới quét file
        time.sleep(1.2)
        quet_tim_file_excel()
    ...
    def bat_dau_xu_ly(e):
        ...
        try:
            write_log(f"\n[1] Đang xử lý file: {selected_file}")
            ...
            write_log("====================================")
            write_log(f"File kết quả: {output_name}")
            quet_tim_file_excel()
    ...
    btn_refresh = ft.ElevatedButton("Bấm để quét lại file", on_click=quet_tim_file_excel)
    btn_run = ft.ElevatedButton("Bắt đầu Tách Sheet ngay", disabled=True, on_click=bat_dau_xu_ly)

    # Đưa các widget lên giao diện trước
    page.add(
        title_text,
        info_text,
        ft.Divider(),
    ...
    # Chạy việc quét file ở một luồng riêng biệt sau khi giao diện đã hiển thị xong
    threading.Thread(target=delayed_scan, daemon=True).start()

# Chạy app dưới dạng kết hợp đảm bảo hiển thị
ft.app(target=main)
