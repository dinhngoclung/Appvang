...
# ===================================================================
# GIAO DIỆN ĐỒ HỌA ỨNG DỤNG (FLET UI APP - BẢN SIÊU TINH GỌN CHỐNG LỖI)
# ===================================================================
def main(page: ft.Page):
    page.title = "Tool Tach Sheet"
    page.scroll = ft.ScrollMode.AUTO
    page.padding = 20

    title_text = ft.Text("TOOL TACH SHEET EXCEL", weight=ft.FontWeight.BOLD)
    selected_file_text = ft.Text("Chua chon file Excel nao", italic=True)
    ...
    def bat_dau_xu_ly(file_path):
        ...
        try:
            ...
            write_log(f"    Duong dan: {file_path}")
            ...
            if len(df) > 0:
                write_log(f" -> Ngay dau: {df.iloc[0]['Ngay'].strftime('%d/%m/%Y')}")
                write_log(f" -> Ngay cuoi: {df.iloc[-1]['Ngay'].strftime('%d/%m/%Y')}")
            else:
                write_log("!!! Khong co du lieu ngay hop le de xu ly!")
    ...
    def pick_files_result(e: ft.FilePickerResultEvent):
        if e.files:
            ...
            selected_file_text.value = f"Da chon: {e.files[0].name}"
            btn_run.disabled = False
            btn_run.data = file_path
            write_log(f"Da nap file: {file_path}")
        else:
            selected_file_text.value = "Huy chon file."
